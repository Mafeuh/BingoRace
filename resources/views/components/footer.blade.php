<div class="text-center bg-white min-h-12">
    ...
    <form method="POST" action="/logout" class="py-2">
        @csrf
        <div class="ml-2 flex gap-2 items-center absolute text-sm">
            {{-- https://nucleoapp.com/svg-flag-icons --}}
            <a href="{{ route('lang.switch', 'fr') }}" @class([
                'bg-emerald-200 rounded-full' => App::getLocale() === 'fr',
                'p-1'
            ])>
            ...
            <a href="{{ route('lang.switch', 'en') }}" @class([
                'bg-emerald-200 rounded-full' => App::getLocale() === 'en',
                'p-1'
            ])>
        ...
        @auth
            <span class="text-sm">{{ __('footer.status.connected_as') }} <b>{{ auth()->user()->name . (auth()->user()->hasPermission('admin') ? ' 🗿' : '') }}</b>
                <button type="submit" class="p-1.5 text-sm rounded-full bg-green-100 text-green-600 font-bold">
                    {{ __('footer.status.logout') }}
                </button>
                @if (auth()->user()->isAdmin())
                    <a href="/admin" class="p-1.5 text-sm rounded-full bg-red-100 font-bold text-red-600">
