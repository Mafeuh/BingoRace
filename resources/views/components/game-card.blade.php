...
<div x-cloak class="relative rounded-2xl transition-all hover:scale-105 text-slate-900"
    ...
    <div>
        <a @if ($redirect) href="/games/{{$game->id}}" @endif 
            class="absolute transition-all inset-0 w-full h-full bg-white/30 dark:hover:bg-slate-900/80 hover:bg-white/80 rounded-2xl flex justify-center items-center">
            <div class="text-center space-y-2">
                <p class="text-xl bg-white/40 p-1 rounded text-black">{{ $game->name }}</p>
                @if ($show_objectives)
                <div class="bg-white/50 p-1">
                    <p>
