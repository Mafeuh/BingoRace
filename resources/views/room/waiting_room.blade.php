...
@section('content')
<x-main-panel class="max-w-6xl mx-auto rounded">
    <div class="select-none">
        <div>
            <h2 class="text-lg text-center dark:text-gray-200" x-data="{ show: false }">
                {{ __('room.wait.code.label') }}
                <span class="text-2xl" x-on:click="show = !show">
                    <span x-show="!show" class="rounded-md font-mono">{{ __('room.wait.code.reveal') }}</span>
                    <span x-show="show" class="font-mono">{{ $room->code }}</span>
                </span>
                <button class="bg-white dark:bg-slate-800 shadow-xl rounded-full py-1 px-2" onclick="copyText()" id="copy_button">
