...
@section('content')
    <div>
        ...
        <i>Pour l'instant, veuillez vous connecter pour rejoindre une partie !</i>
        <form action="/join" method="POST" class="flex">
            <input type="text" name="username" id="username" placeholder="Nom d'utilisateur" class="w-44 border-0 rounded-l-xl shadow-xl" disabled>
            <input type="text" name="username" id="username" placeholder="Code" class="text-center w-20 border-0 border-x shadow-xl" disabled>
            <button type="submit" class="
            flex items-center px-3 bg-green-100 rounded-r-xl shadow-xl
            hover:bg-green-600 hover:text-white hover:font-bold
            " disabled>Rejoindre</button>
        </form>
